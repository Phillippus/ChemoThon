"""Pydantic schéma znalostnej databázy (knowledge base, KB).

Každý záznam v KB je jedno odporúčanie jednej odbornej spoločnosti pre jednu
kombináciu klinicko-patologických kritérií. Schéma vynucuje povinné polia vrátane
audit-trailu (`generated_by`) a kontrolného stavu (`reviewed`).
...
class Recommendation(BaseModel):
    ...
    a tá je v zozname. Prázdne `criteria` = záznam platí pre celú entitu (všeobecný).
    ...
    # Voliteľné polia z klinického auditu (2026-06-22, Cowork). Nepovinné — nepoužívajú sa
    # na matching; slúžia na sledovanie stavu kontroly a novších guidelines pre Fázu 3.
    ...
    def _check_review_consistency(self) -> "Recommendation":
        # Ak je záznam označený ako overený, MUSÍ mať vyplneného kontrolóra a dátum.
        if self.reviewed:
