"""MDM Gyn — nástroj klinickej podpory rozhodovania pre gynekologickú onkológiu.
...
# Zabezpeč import balíka `mdmgyn` nezávisle od pracovného adresára (Streamlit Cloud).
...
import mdmgyn.schema as _schema  # noqa: E402
importlib.reload(_schema)
import mdmgyn.entities as _entities  # noqa: E402
importlib.reload(_entities)
import mdmgyn.matching as _matching  # noqa: E402
...
DISCLAIMER = (
    "**MDM Gyn** je nástroj klinickej podpory rozhodovania **vo vývoji**. Všetky "
...
SOCIETY_ORDER = [Society.ESMO, Society.ESGO, Society.ESTRO, Society.NCCN]


# --------------------------------------------------------------------------- #
# Načítanie KB (cache na úrovni session — KB sa pri runtime nemení).
# --------------------------------------------------------------------------- #
...
def render_citation(rec) -> None:
    """Rozbaľovacia citácia priamo pri odporúčaní + stav kontroly."""
...
def render_recommendation_card(rec) -> None:
    ...
    meta = []
    if rec.figo_stage:
        meta.append(f"FIGO {rec.figo_stage}")
    if rec.risk_group:
        meta.append(f"riziko: {rec.risk_group}")
    if meta:
...
def page_home() -> None:
    st.title("MDM Gyn")
    st.markdown(
        "Nástroj klinickej podpory rozhodovania pre **gynekologickú onkológiu** — "
        "FIGO štádium, riziková stratifikácia a odporúčaný postup podľa **ESMO, ESGO, "
        "ESTRO a NCCN**."
    )
    show_disclaimer()
    st.subheader("Vyber entitu")
    cols = st.columns(2)
...
def page_review_status() -> None:
    ...
    st.markdown(
        "Prehľad overených (`reviewed: true`) a neoverených záznamov per entita a spoločnosť. "
    ...
    st.subheader("Podľa entity a spoločnosti")
    rows = []
...
# --------------------------------------------------------------------------- #
def main() -> None:
    st.set_page_config(page_title="MDM Gyn", page_icon="⚕️", layout="centered")
    ...
    # `nav` v session_state riadi výber (nastavuje ho aj tlačidlo na Domove).
    ...
    st.sidebar.divider()
    st.sidebar.caption("MDM Gyn · Fáza 1 (scaffold)")
