"""MDM Colorectal — nástroj klinickej podpory rozhodovania pre kolorektálny karcinóm.
...
# Zabezpeč import balíka `mdmcolorectal` nezávisle od pracovného adresára (Streamlit Cloud).
...
import mdmcolorectal.schema as _schema  # noqa: E402
importlib.reload(_schema)
import mdmcolorectal.entities as _entities  # noqa: E402
importlib.reload(_entities)
import mdmcolorectal.matching as _matching  # noqa: E402
...
DISCLAIMER = (
    "**MDM Colorectal** je nástroj klinickej podpory rozhodovania **vo vývoji**. Všetky "
...
SOCIETY_ORDER = [Society.ESMO, Society.NCCN, Society.ASCO]
...
# --------------------------------------------------------------------------- #
def page_home() -> None:
    st.title("MDM Colorectal")
    st.markdown(
        "Nástroj klinickej podpory rozhodovania pre **kolorektálny karcinóm** — štádium "
        "(AJCC TNM 8), riziková stratifikácia a odporúčaný postup podľa **ESMO, NCCN "
        "a ASCO**. Členené na **kolon, rektum** a **metastatické** ochorenie."
    )
    show_disclaimer()
...
# --------------------------------------------------------------------------- #
def main() -> None:
    st.set_page_config(page_title="MDM Colorectal", page_icon="🟤", layout="centered")
    ...
    st.sidebar.divider()
    st.sidebar.caption("MDM Colorectal · Fáza 1 (scaffold)")
