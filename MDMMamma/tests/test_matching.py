...
def _rec(rec_id, society, criteria, entity=Entity.EARLY):
...
def test_full_criteria_match():
    ...
    hits = matching.match(recs, Entity.EARLY, {"subtype": "tnbc"})
...
def test_partial_input_does_not_match():
    ...
    hits = matching.match(recs, Entity.EARLY, {"subtype": "luminal_a", "nodal": ""})
    assert hits == []


def test_value_not_in_allowed_does_not_match():
    recs = [_rec("a", Society.ESMO, {"subtype": ["tnbc"]})]
    hits = matching.match(recs, Entity.EARLY, {"subtype": "luminal_a"})
    assert hits == []
...
def test_module_isolation():
    ...
    hits = matching.match(recs, Entity.EARLY, {})
    assert hits == []
...
def test_specificity_ordering():
    ...
    hits = matching.match(recs, Entity.EARLY, {"subtype": "luminal_a", "nodal": "negative"})
    assert hits[0].id == "specific"


def test_seed_kb_early_tnbc_matches_esmo():
    kb = load_kb()
    hits = matching.match(kb, Entity.EARLY, {"subtype": "tnbc"})
