    ...
    if 'bsa' in st.session_state:
        ...
        if st.button("Display Protocol") and selected_protocol_name.strip():
            if selected_protocol_name == "Enfortumab Vedotin + Pembrolizumab (EV-302, 1L metastatic urothelial)":
                ...
                tax_dose = round(175 * bsa, 2)
                ifo_dose = round(1200 * bsa, 2)
                ddp_dose = round(20 * bsa, 2)
                mesna_dose = round(400 * bsa, 2)
                st.write("### TIP (Paclitaxel + Ifosfamide + Cisplatin) — salvage germ cell tumors")
                st.write(f"paclitaxel 175 mg/m² ......... {tax_dose} mg D1")
                st.write(f"cisplatin 20 mg/m² ......... {ddp_dose} mg D1-5")
                st.write(f"ifosfamide 1200 mg/m² ......... {ifo_dose} mg D1-5")
                st.write(f"mesna 400 mg/m² ......... {mesna_dose} mg D1-5 (plus 2 more doses after ifosfamide, see below)")
                ...
                st.write(f"paclitaxel {tax_dose} mg in 500 ml normal saline i.v. over 3 hours")
                st.write("#### D1-D5 Premedication")
                st.write("Granisetron 2 mg p.o., Dexamethasone 12 mg i.v., Pantoprazole 40 mg p.o.")
                st.write(f"Mesna {mesna_dose} mg i.v. before ifosfamide, then {mesna_dose} mg at 4h and {mesna_dose} mg at 8h after ifosfamide (3 × {mesna_dose} mg total), D1-D5. Hydration: NaCl 1000 ml before and after ifosfamide.")
                st.write("#### D1-D5 Chemotherapy (in this order)")
                st.write(f"ifosfamide {ifo_dose} mg in 1000 ml normal saline i.v./1-2h (with mesna)")
                vials = int(ddp_dose // 50); rem = round(ddp_dose % 50, 1)
                for i in range(vials):
                ...
                if rem > 0:
                    st.write(f"cisplatin {rem} mg in 500 ml normal saline i.v.")
                st.write("Mannitol 10% 250 ml i.v.")
                st.write("Supportive care: Pegfilgrastim 6 mg s.c. D6.")
