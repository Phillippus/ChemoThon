"""MDM Haematology — nástroj klinickej podpory rozhodovania pre lymfómy.

Pokrýva lymfómy (NIE leukémie). Streamlit entry point. Spustenie lokálne:
...
# Zabezpeč import balíka `mdmhaematology` nezávisle od pracovného adresára (Streamlit Cloud).
...
from mdmhaematology.schema import UNVERIFIED_MARK, Entity, Society  # noqa: E402

DISCLAIMER = (
    "**MDM Haematology** je nástroj klinickej podpory rozhodovania **vo vývoji**. Všetky "
...
SOCIETY_ORDER = [Society.ESMO, Society.NCCN, Society.EHA]
...
# --------------------------------------------------------------------------- #
def page_home() -> None:
    st.title("MDM Haematology")
    st.markdown(
        "Nástroj klinickej podpory rozhodovania pre **lymfómy** (nie leukémie) — štádium "
        "(Lugano), riziková stratifikácia a odporúčaný postup podľa **ESMO, NCCN a EHA**. "
        "Moduly: Hodgkinov lymfóm, DLBCL, folikulárny, z plášťových buniek, "
        "marginálnozónový a periférny T-bunkový lymfóm."
    )
    show_disclaimer()
    st.subheader("Vyber typ lymfómu")
    cols = st.columns(2)
...
def page_entity(entity: Entity) -> None:
    ...
    # Štádium (zadané používateľom; auto-derivácia nie je vo Fáze 1).
    stage = inputs.get("stage", UNKNOWN)
    if "stage" in {f.key for f in ed.fields}:
        if stage:
            st.markdown(f"**Štádium (Lugano):** {stage}")
        else:
...
- Prognostické indexy (IPI/FLIPI/MIPI/IPS) sa **nepočítajú** — zadávajú sa ako vstup.
...
# --------------------------------------------------------------------------- #
def main() -> None:
    st.set_page_config(page_title="MDM Haematology", page_icon="🩸", layout="centered")
    ...
    st.sidebar.divider()
    st.sidebar.caption("MDM Haematology · Fáza 1 (scaffold)")
