"""Validačné testy znalostnej databázy (KB) — lymfómy.
...
from mdmhaematology.entities import ENTITIES
from mdmhaematology.kb_loader import load_kb
from mdmhaematology.schema import UNVERIFIED_MARK, Recommendation
...
def test_all_entities_have_seed_records(kb):
    """Každý modul má aspoň jeden seed záznam (pokrytie wizardu)."""
...
def test_missing_required_field_fails_validation():
    incomplete = {"id": "x", "entity": "hl", "society": "ESMO"}
...
def test_reviewed_true_without_reviewer_fails():
    with pytest.raises(Exception):
        Recommendation(
            id="x", entity="hl", society="ESMO",
            guideline_version="v1", year=2018, source_reference="ref",
