"""MDM Prostata — NCCN riziková stratifikácia karcinómu prostaty.

Streamlit entry point. Spustenie lokálne:
    streamlit run app.py

ARCHITEKTÚRA PROTI HALUCINÁCII: aplikácia NEGENERUJE klinický obsah za behu.
Riziková skupina je odvodená čisto deterministickou funkciou (mdmprostate/risk.py)
...
from mdmprostate.risk import (  # noqa: E402
    NCCN_SOURCE_LABEL,
    ProstateInputs,
    T_STAGES,
    classify,
...
DISCLAIMER = (
    "**MDM Prostata** je nástroj klinickej podpory rozhodovania **vo vývoji**. "
...
def page_classifier() -> None:
    st.title("🎯 MDM Prostata — NCCN riziková stratifikácia")
    show_disclaimer()
    ...
    with st.form("prostate_form"):
        ...
        with col2:
            ...
            gleason_primary = st.selectbox("Gleason primárny pattern", [3, 4, 5], index=0)
            gleason_secondary = st.selectbox("Gleason sekundárny pattern", [3, 4, 5], index=0)
...
- Aplikácia **negeneruje** klinický obsah za behu pomocou LLM. Riziková skupina je
  čisto **deterministická funkcia** vstupných parametrov (`mdmprostate/risk.py`).
...
prostaty (cT1a–T4, N0–N1, M0–M1) do NCCN rizikových skupín. Nepokrýva terapeutické
odporúčania ani nomogramy (napr. CAPRA, MSKCC) — len zaradenie do rizikovej skupiny.
"""
    )


def main() -> None:
    st.set_page_config(page_title="MDM Prostata", page_icon="🎯", layout="centered")
    pages = ["🎯 Klasifikátor", "ℹ️ O aplikácii"]
    choice = st.sidebar.radio("Navigácia", pages, key="nav")
    st.sidebar.divider()
    st.sidebar.caption("MDM Prostata · Fáza 1 (scaffold)")
